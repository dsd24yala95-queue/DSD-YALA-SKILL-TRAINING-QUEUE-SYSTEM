...
const emptyBranch: Omit<MasterBranch, "id"> = {
    branchName: "",
    levels: "1,2,3",
    maxQueue: 20,
    Date: "",
...
export default function AdminTestingPage() {
    ...
    const [editTarget, setEditTarget] = useState<MasterBranch | null>(null);
    ...
    const loadBranches = useCallback(async () => {
        ...
        try {
            ...
            toast.error("ไม่สามารถโหลดข้อมูลสาขาทดสอบได้");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const openEdit = (branch: MasterBranch) => {
    ...
    const handleSave = async () => {
        if (!form.branchName?.trim()) {
            toast.error("กรุณากรอกชื่อสาขาทดสอบมาตรฐาน");
            return;
        }
        ...
        try {
            ...
            toast.success(editTarget ? "บันทึกการแก้ไขสาขาทดสอบแล้ว" : "เพิ่มสาขาทดสอบใหม่เรียบร้อย");
            setModalOpen(false);
            loadBranches();
        } catch {
    ...
    const handleDelete = async (id: string) => {
        try {
            const res = await fetch(`/api/master/branches?id=${id}`, { method: "DELETE" });
            if (!res.ok) throw new Error("Failed to delete");
            toast.success("ลบสาขาทดสอบเรียบร้อยแล้ว");
            setDeleteConfirm(null);
            loadBranches();
        } catch {
            toast.error("เกิดข้อผิดพลาดในการลบสาขาทดสอบ");
        }
    };
    ...
    const totalMaxQueue = branches.reduce((acc, b) => acc + (b.maxQueue || 0), 0);
    ...
    return (
        <div className="p-4 sm:p-6 bg-[#f8fafc] min-h-screen font-sans">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                <div>
                    <h1 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3">
                        ...
                        การทดสอบมาตรฐาน (Standard Testing Management)
                    </h1>
                    <p className="text-xs text-slate-400 mt-1">
                        จัดการข้อมูลสาขาทดสอบมาตรฐานฝีมือแรงงานแห่งชาติ สพร.24 ยะลา
                    </p>
                </div>
                ...
                >
                    <i className="fa-solid fa-plus"></i> เพิ่มสาขาทดสอบใหม่
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                {[
                    { label: "สาขาทดสอบทั้งหมด", value: branches.length, icon: "fa-clipboard-check", color: "from-blue-500 to-indigo-600" },
                    { label: "เปิดรับสมัครอยู่", value: activeCount, icon: "fa-circle-check", color: "from-emerald-500 to-teal-600" },
                    { label: "คิวสูงสุดรวมต่อวัน", value: `${totalMaxQueue} คิว/วัน`, icon: "fa-users", color: "from-indigo-500 to-purple-600" },
                ].map((c, i) => (
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm mb-6 flex flex-col md:flex-row gap-3 items-center justify-between">
                <div className="relative w-full md:w-80">
                    ...
                    <input
                        type="text"
                        placeholder="ค้นหาชื่อสาขาทดสอบ, สถานที่สอบ..."
                        value={search}
            ...
            {loading ? (
                <div className="py-24 flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs text-slate-400 font-medium">กำลังโหลดข้อมูลสาขาทดสอบ...</p>
                </div>
            ) : filteredBranches.length === 0 ? (
                <div className="bg-white rounded-3xl border border-slate-100 py-16 text-center text-slate-400 p-4">
                    <i className="fa-solid fa-clipboard-check text-4xl mb-3 block opacity-30"></i>
                    <p className="text-sm font-semibold mb-3">ยังไม่มีข้อมูลสาขาทดสอบมาตรฐานตามเงื่อนไข</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    <AnimatePresence>
                        {filteredBranches.map((b, i) => {
                            const max = b.maxQueue || 20;
                            const current = b.currentQueue || 0;
                                ...
                                >
                                    <div>
                                        <div className="flex items-start justify-between mb-3">
                                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-sm text-white text-sm">
                                                <i className="fa-solid fa-tools"></i>
                                            </div>
                                            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${b.status === "active" ? "bg-emerald-500/10 text-emerald-600 border-emerald-400/20" : "bg-slate-100 text-slate-400 border-slate-200"}`}>
                                                {b.status === "active" ? "เปิดรับสมัคร" : "ปิดรับสมัคร"}
                                            </span>
                                        </div>

                                        <h3 className="font-bold text-slate-800 text-sm sm:text-base leading-snug mb-2">{b.branchName}</h3>
                                        ...
                                        <div className="space-y-2 mt-3">
                                            ...
                                            <p className="text-xs font-semibold text-slate-600 flex items-center gap-2 bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-100">
                                                <i className="fa-regular fa-calendar-days text-indigo-500 shrink-0"></i>
                                                <span>{formatDateRangeTh(b.Date, b.DateEnd)}</span>
                                            </p>

                                            <p className="text-[11px] text-slate-500 flex items-center gap-2">
                                                <i className="fa-solid fa-layer-group w-4 text-blue-400"></i>
                                                <span>ระดับทักษะ {b.levels}</span>
                                            </p>
                                            ...
                                            <div>
                                                <a
                                                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(b.LocationGPS || b.LocationName || "สถาบันพัฒนาฝีมือแรงงาน 24 ยะลา")}`}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200/60 text-[11px] font-bold transition-all"
                                                >
                                            ...
                                            <div className="bg-slate-50 rounded-xl p-2.5 border border-slate-100 mt-2">
                                                <div className="flex justify-between text-[11px] font-bold mb-1">
                                                    <span className="text-slate-500">ผู้สมัครทดสอบ:</span>
                                                    <span className={
                                                        percent >= 100
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setApplicantModalTarget({ id: b.id, name: b.branchName })}
                                    ...
                                    {deleteConfirm === b.id ? (
                                        <div className="flex gap-1.5">
                                            <button onClick={() => handleDelete(b.id)} className="py-2 px-3 rounded-xl bg-red-600 text-white text-xs font-bold shadow-sm">ยืนยัน</button>
                                        ...
                                        <button onClick={() => setDeleteConfirm(b.id)} className="py-2 px-3 rounded-xl border border-red-100 text-xs font-bold text-red-400 hover:bg-red-50 transition-all">
                        ...
                        <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-6 sm:p-7 max-h-[90vh] overflow-y-auto my-8">
                            <div className="flex items-center justify-between mb-5 pb-3 border-b border-slate-100">
                                <h2 className="text-base sm:text-lg font-black text-slate-800 flex items-center gap-2">
                                    <i className="fa-solid fa-clipboard-check text-indigo-600"></i>
                                    {editTarget ? "✏️ แก้ไขสาขาทดสอบ" : "➕ เพิ่มสาขาทดสอบใหม่"}
                                </h2>
                            ...
                            <div className="space-y-3.5 text-xs">
                                <div>
                                    <label className="font-bold text-slate-600 block mb-1">ชื่อสาขาทดสอบมาตรฐาน <span className="text-rose-500">*</span></label>
                                    <input
                                        type="text"
                                        placeholder="เช่น ช่างไฟฟ้าภายในอาคาร"
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                        <label className="font-bold text-slate-600 block mb-1">ระดับทักษะ (เช่น 1, 2, 3)</label>
                                        <input
                                            type="text"
                                            placeholder="1, 2"
                                            value={form.levels || "1,2,3"}
                                    ...
                                    <div>
                                        <label className="font-bold text-slate-600 block mb-1">จำนวนคิวสูงสุดต่อวัน</label>
                                        <input
                                            type="number"
                                            min={1}
                                            value={form.maxQueue || 20}
                                            onChange={(e) => setForm({ ...form, maxQueue: Number(e.target.value) })}
                                            className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-400/30"
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label className="font-bold text-slate-600 block mb-1">สถานที่สอบ</label>
                                    <input
                                        type="text"
                                        placeholder="เช่น ศูนย์ทดสอบมาตรฐาน สพร.24 ยะลา"
                            ...
                            <div className="flex gap-3 mt-6 pt-4 border-t border-slate-100">
                                ...
                                <button onClick={handleSave} disabled={saving} className="flex-1 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition-all disabled:opacity-50 shadow-md shadow-indigo-500/20 active:scale-95">
                                    {saving ? "กำลังบันทึก..." : "💾 บันทึกข้อมูลสาขาทดสอบ"}
                                </button>
                            </div>
                        </motion.div>
                    </motion.div>
